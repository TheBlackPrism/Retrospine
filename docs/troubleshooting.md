...
| Connecting Tolino Cloud fails with "The bookshop rejected the refresh token" | The token was already used (refresh tokens rotate, and the web reader tab uses its copy when it stays open), expired, or the web reader session was signed out | Sign in to the web reader again, copy a fresh `refresh_token` from the `token` request, close that tab without signing out and connect right away |
| Connecting fails with "Signed in to …, but … failed" | The token worked; the Tolino Cloud refused the device registration or the library request that follows | The message names the step and the Tolino Cloud's answer. For a device limit, remove a device in the web reader; otherwise try the *Advanced* device id from the web reader's Network tab |
| Settings say "Tokens are renewed by your browser" | The bookshop's bot protection (Cloudflare) blocks token requests from non-browser clients such as the server, so Retrospine runs the connection in browser mode | Nothing to fix; syncs run while Retrospine is open in a browser and for about an hour afterwards. See [tolino-sync.md](tolino-sync.md) |
| Connecting fails with "blocked the token request from your browser" or "Your browser could not reach …" | The bookshop refused the request from the browser as well, usually because a content blocker or privacy extension stopped it | Allow the bookshop's domain in the blocker or try another browser |
